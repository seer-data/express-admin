...
function select (args) {
    ...
    args.filter.searchColumns = view.searchColumns || [];
...
function statement (table, columns, filter, joins) {
	...
	for (var i=0; i < filter.searchColumns.length; i++) {
		if (filter.searchText) {
			var expr = expression(table, filter.searchColumns[i], {"textarea": true}, filter.searchText);
			if (!expr) continue;
			filter.or = true
			statements.push(expr);
		}
...
function expression (table, column, control, value) {
    ...
    if (control.select && !!control.options) {
    	...
    	// hardcoding 'ilike' as xsql doesn't support it
        return [name,' ilike '+x.string('%'+value+'%')].join(' ');
    }
    else {
        return x.eq(name,value);
    }
